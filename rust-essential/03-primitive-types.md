...
说到数组，我们就不能不提Rust中提供的另一种基础数据类型：切片。切片实际上也是一个胖指针，包含一个地址（指向栈或者堆均可）加上一段长度，用以标记某个数据中的一部分（例如数组中的一部分）：
